...
export function DocumentCard({ document }: Props) {
  return (
    <div className="bg-zinc-800 rounded-xl overflow-hidden">
      ...
      <div className="p-4">
        <h3 className="text-white font-semibold">{document.documentType} encontrado en {document.location}</h3>
        <p className="text-zinc-400 text-sm mt-1">{document.publicationDate} • {document.location}</p>
        <Link to={`/chat/${document.id}`}>
          <button className="mt-4 w-full bg-green-500 text-white py-2 rounded-lg font-semibold">
            Contactar
          </button>
        </Link>
      </div>
